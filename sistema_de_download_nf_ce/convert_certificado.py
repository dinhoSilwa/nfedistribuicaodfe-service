...
from sistema_de_download_nf_ce.config import CERT_PASSWORD, CERT_PFX_PATH
...
def converter_pfx_para_pem(pfx_path: Path, senha: str) -> tuple[Path, Path]:
    ...
    """
    perm_dir = Path("perm")
    perm_dir.mkdir(exist_ok=True)  # ← cria a pasta 'perm' na raiz do projeto
    cert_pem = perm_dir / "cert.pem"
    key_pem = perm_dir / "key.pem"
